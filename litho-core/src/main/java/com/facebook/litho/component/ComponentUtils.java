...
public class ComponentUtils {
    ...
    private static boolean areComponentCollectionsEquals(
        int level, Collection c1, Collection c2) {
        if (level < 1) {
     ...
     */
    public static String treeToString(@Nullable InternalNode root) {
        if (root == null) {
            return "null";
        }

        StringBuilder       builder = new StringBuilder();
        Deque<InternalNode> stack   = new LinkedList<>();
        stack.addLast(null);
        stack.addLast(root);
        int level = 0;
        while (!stack.isEmpty()) {
            InternalNode node = stack.removeLast();
            if (node == null) {
                level--;
                continue;
            }
            ...
            if (node != root) {
                builder.append('\n');
                boolean                      isLast;
                Iterator<InternalNode> iterator = stack.iterator();
                iterator.next();
                iterator.next();
