...
import static com.facebook.litho.component.ComponentKeyUtils.getKeyForChildPosition;
...
public abstract class Component extends ComponentLifecycle
     ...
     */
    public @Nullable List<WorkingRangeContainer.Registration> mWorkingRangeRegistrations;
    ...
    public String getSimpleName() {
        Component delegate = getSimpleNameDelegate();
        if (delegate == null) {
            return mSimpleName;
        }
    ...
    @Nullable public StateContainer getStateContainer() {
        return null;
    }
    ...
    private static void assertSameBaseContext(
        ...
        if (scopedContext.getAndroidContext() != willRenderContext.getAndroidContext()) {
            ComponentsLogger logger = scopedContext.getLogger();
            if (logger != null) {
                logger.emitMessage(
    ...
    @Nullable public String getOwnerGlobalKey() {
    ...
    private String generateUniqueGlobalKeyForChild(Component component, String key) {
        String     childKey   = ComponentKeyUtils.getKeyWithSeparator(getGlobalKey(), key);
        KeyHandler keyHandler = mScopedContext.getKeyHandler();
    ...
    private int getChildCountAndIncrement(Component component) {
        ...
        int count  = mChildCounters.get(typeId, 0);
    ...
    private void logDuplicateManualKeyWarning(Component component, String key) {
        ComponentsLogger logger = mScopedContext.getLogger();
        if (logger != null) {
            logger.emitMessage(
    ...
    public Component makeShallowCopyWithNewId() {
        Component component = makeShallowCopy();
        component.mId = sIdGenerator.incrementAndGet();
        return component;
    }

    public Component makeUpdatedShallowCopy(ComponentContext c) {
    ...
    public InternalNode consumeLayoutCreatedInWillRender() {
        InternalNode layout = mLayoutCreatedInWillRender;
    ...
    public void updateInternalChildState(ComponentContext parentContext) {
        if (ComponentsConfiguration.isDebugModeEnabled || ComponentsConfiguration.useGlobalKeys) {

            boolean isRefactoredKeyGenerationEnabled = parentContext.isReconciliationEnabled();
    ...
    public Component getThreadSafeInstance() {
        ...
        boolean shouldCreateNewInstance = mLayoutVersionGenerator.getAndSet(true);
    ...
    public void bindDynamicProp(int dynamicPropIndex, Object value, Object content) {
    ...
    public abstract static class Builder<T extends Builder<T>> {
        ...
        public T key(@Nullable String key) {
            if (key == null) {
                ComponentsLogger logger = mContext.getLogger();
                if (logger != null) {
