...
public class StateHandler {
     ...
     */
    public synchronized void queueStateUpdate(
     ...
     */
    @ThreadSafe(enableChecks = false) public void applyStateUpdatesForComponent(Component component) {
        ...
        List<StateUpdate> stateUpdatesForKey;
        ...
        if (stateUpdatesForKey != null) {
            for (StateUpdate update : stateUpdatesForKey) {
                ...
                Transition transition = obtainTransitionFromStateContainer(stateContainer);
                if (transition != null) {
    ...
    public static void maybeCreateInitialStateAndCommitResult(
        ...
        synchronized (initLock) {
            Object lock = stateHandler.getCreateInitialStateLock(key, initLock);
     ...
     */
    public void commit(StateHandler stateHandler, boolean isReconciliationEnabled) {
        clearStateUpdates(
    ...
    private void clearStateUpdates(@Nullable Map<String, List<StateUpdate>> appliedStateUpdates) {
        ...
        for (Map.Entry<String, List<StateUpdate>> appliedStateUpdate : appliedStateUpdates.entrySet()) {
            ...
            List<StateUpdate> pendingStateUpdatesForKey;
            List<StateUpdate> pendingLazyStateUpdatesForKey;
            synchronized (this) {
            ...
            List<StateUpdate> appliedStateUpdatesForKey = appliedStateUpdate.getValue();
    ...
    public synchronized Map<String, StateContainer> getStateContainers() {
        return mStateContainers;
    }

    @Nullable
    public synchronized Map<String, List<StateUpdate>> getPendingStateUpdates() {
        return mPendingStateUpdates;
    }

    @Nullable
    public synchronized Map<String, List<StateUpdate>> getPendingLazyStateUpdates() {
    ...
    @VisibleForTesting
    public synchronized Map<String, List<StateUpdate>> getAppliedStateUpdates() {
        return mAppliedStateUpdates;
    }

    public synchronized void consumePendingStateUpdateTransitions(
    ...
    @Nullable
    public synchronized Object getCachedValue(Object cachedValueInputs) {
    ...
    public synchronized void putCachedValue(Object cachedValueInputs, Object cachedValue) {
        if (mCachedValues == null) {
            mCachedValues = new HashMap<>();
        }
