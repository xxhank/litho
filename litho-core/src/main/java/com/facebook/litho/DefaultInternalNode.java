...
import com.facebook.litho.geometry.Edges;
...
import static com.facebook.litho.component.ComponentsLogger.LogLevel.WARNING;
...
public class DefaultInternalNode implements InternalNode, Cloneable {
        ...
        if ((mPrivateFlags & PFLAG_PADDING_IS_SET) != 0L) {
            ...
            for (int i = 0; i < Edges.EDGES_LENGTH; i++) {
                ...
                if (!YogaConstants.isUndefined(value)) {
                    YogaEdge edge = YogaEdge.fromInt(i);
    ...
    public void assertContextSpecificStyleNotSet() {
        ...
        if (errorTypes != null) {
            CharSequence     errorStr = TextUtils.join(", ", errorTypes);
            ComponentsLogger logger   = getContext().getLogger();
            if (logger != null) {
                logger.emitMessage(
                    WARNING,
                    "You should not set "
    ...
    public DefaultInternalNode deepClone() {
        ...
        // 2. Clone this layout.
        DefaultInternalNode copy = clone();
    ...
    @Override protected DefaultInternalNode clone() {
        try {
    ...
    private float resolveHorizontalEdges(Edges spacing, YogaEdge edge) {
        boolean isRtl = (mYogaNode.getLayoutDirection() == YogaDirection.RTL);
    ...
    private static InternalNode reconcile(
        ...
        DefaultInternalNode layout = getCleanUpdatedShallowCopy(c, current, next, copiedNode);
    ...
    static @ReconciliationMode int getReconciliationMode(
        ...
        Component       root       = current.getHeadComponent();
    ...
    @IntDef({ReconciliationMode.COPY, ReconciliationMode.RECONCILE, ReconciliationMode.RECREATE}) @interface ReconciliationMode {
        int COPY      = 0;
        int RECONCILE = 1;
        int RECREATE  = 2;
    }
