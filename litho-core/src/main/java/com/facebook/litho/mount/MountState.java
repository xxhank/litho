...
package com.facebook.litho.mount;
...
import com.facebook.litho.event.EventDispatcherUtils;
...
import com.facebook.litho.test.TestOutput;
...
import static com.facebook.litho.component.Component.isHostSpec;
import static com.facebook.litho.component.Component.isMountViewSpec;
import static com.facebook.litho.component.ComponentHostUtils.maybeInvalidateAccessibilityState;
import static com.facebook.litho.component.ComponentHostUtils.maybeSetDrawableState;
...
public class MountState implements TransitionManager.OnAnimationCompleteListener {
    ...
    public void mount(
        ...
        if (!isIncrementalMountEnabled
            ...
            MountItem rootMountItem = mIndexToItemMap.get(ROOT_HOST_ID);
            ...
            for (int i = 0, size = layoutState.getMountableOutputCount(); i < size; i++) {
                ...
                MountItem currentMountItem = getItemAt(i);
                ...
                if (isMountable && !isMounted) {
                    ...
                    if (isAnimationLocked(i) && isIncrementalMountEnabled && component.hasChildLithoViews()) {
                        ...
                        View view = (View) getItemAt(i).getContent();
    ...
    private void maybeUpdateAnimatingMountContent() {
        ...
        // TransitionManager
        Map<TransitionId, OutputUnitsAffinityGroup<Object>> animatingContent =
            new LinkedHashMap<>(mAnimatingTransitionIds.size());
        for (int i = 0, size = mIndexToItemMap.size(); i < size; i++) {
            MountItem mountItem = mIndexToItemMap.valueAt(i);
            if (!mountItem.hasTransitionId()) {
                continue;
            }
            long                             layoutOutputId = mIndexToItemMap.keyAt(i);
            @OutputUnitType int              type           = LayoutStateOutputIdCalculator.getTypeFromId(layoutOutputId);
    ...
    public void processVisibilityOutputs(
        ...
        boolean isDoingPerfLog = mMountStats.isLoggingEnabled;
        boolean isTracing      = ComponentsSystrace.isTracing();
        ...
        for (int j = 0, size = layoutState.getVisibilityOutputCount(); j < size; j++) {
            ...
            long                       handlerStartTime = isDoingPerfLog ? System.nanoTime() : 0;
            ...
            if (isCurrentlyVisible) {
                ...
                if (visibilityChangedHandler != null) {
                    int visibleWidth  = sTempRect.right - sTempRect.left;
                    int visibleHeight = sTempRect.bottom - sTempRect.top;
                    EventDispatcherUtils.dispatchOnVisibilityChanged(
    ...
    private void processTestOutputs(LayoutState layoutState) {
        ...
        for (int i = 0, size = layoutState.getTestOutputCount(); i < size; i++) {
            ...
            MountItem  mountItem      = layoutOutputId == -1 ? null : mIndexToItemMap.get(layoutOutputId);
            ...
            Deque<TestItem> items        = mTestItemMap.get(testOutput.getTestKey());
            Deque<TestItem> updatedItems = items == null ? new LinkedList<TestItem>() : items;
    ...
    private boolean isMountedHostWithChildContent(MountItem mountItem) {
        ...
        ComponentHost host = (ComponentHost) content;
    ...
    public List<LithoView> getChildLithoViewsFromCurrentlyMountedItems() {
        ArrayList<LithoView> childLithoViews = new ArrayList<>();
        for (int i = 0; i < mIndexToItemMap.size(); i++) {
            long      layoutOutputId = mIndexToItemMap.keyAt(i);
            MountItem mountItem      = mIndexToItemMap.get(layoutOutputId);
    ...
    public void clearVisibilityItems() {
        ...
        for (int i = mVisibilityIdToItemMap.size() - 1; i >= 0; i--) {
            VisibilityItem visibilityItem = mVisibilityIdToItemMap.valueAt(i);
    ...
    private boolean isInFocusedRange(Rect componentBounds, Rect componentVisibleBounds) {
        View parent = (View) mLithoView.getParent();
        if (parent == null) {
            return false;
        }

        int halfViewportArea     = parent.getWidth() * parent.getHeight() / 2;
        int totalComponentArea   = computeRectArea(componentBounds);
        int visibleComponentArea = computeRectArea(componentVisibleBounds);
    ...
    private boolean updateMountItemIfNeeded(
        ...
        if (shouldUpdate) {
            ...
            if (mLastMountedComponentTreeId != componentTreeId) {
                Component component = currentMountItem.getComponent();

                if (isHostSpec(component)) {
                    ComponentHost componentHost = (ComponentHost) currentMountItem.getContent();
            ...
            ComponentHost host = currentMountItem.getHost();
    ...
    private static boolean shouldUpdateMountItem(
        ...
        MountItem currentMountItem,
        boolean useUpdateValueFromLayoutOutput) {
        @LayoutOutput.UpdateState int updateState      = layoutOutput.getUpdateState();
    ...
    private void prepareMount(LayoutState layoutState, @Nullable PerfEvent perfEvent) {
        List<Integer>     disappearingItems = extractDisappearingItems(layoutState);
        PrepareMountStats stats             = unmountOrMoveOldItems(layoutState, disappearingItems);
    ...
    private boolean isItemDisappearing(LayoutState layoutState, int index) {
        ...
        LayoutOutput layoutOutput = mLastMountedLayoutState.getMountableOutputAt(index);
    ...
    private List<Integer> extractDisappearingItems(LayoutState newLayoutState) {
        ...
        while (index < mLayoutOutputsIds.length) {
            if (isItemDisappearing(newLayoutState, index)) {
                int lastDescendantIndex = findLastDescendantIndex(mLastMountedLayoutState, index);
                ...
                for (int j = index; j <= lastDescendantIndex; j++) {
                    MountItem mountedItem = getItemAt(j);
                    if (mountedItem != null) {
                        // Item is already mounted - skip
                        continue;
                    }
                    LayoutOutput layoutOutput = mLastMountedLayoutState.getMountableOutputAt(j);
                    mountLayoutOutput(j, layoutOutput, mLastMountedLayoutState);
                }

                // Reference to the root of the disappearing subtree
                MountItem disappearingItem = getItemAt(index);
    ...
    private void remountComponentHostToRootIfNeeded(int index) {
        ComponentHost rootHost = mHostsByMarker.get(ROOT_HOST_ID);
    ...
    private PrepareMountStats unmountOrMoveOldItems(
        ...
        for (int i = 0; i < mLayoutOutputsIds.length; i++) {
            LayoutOutput newLayoutOutput = newLayoutState.getLayoutOutput(mLayoutOutputsIds[i]);
            int          newPosition     = newLayoutOutput == null ? -1 : newLayoutOutput.getIndex();

            MountItem oldItem = getItemAt(i);
    ...
    private void mountLayoutOutput(int index, LayoutOutput layoutOutput, LayoutState layoutState) {
        ...
        ComponentContext context = getContextForComponent(component);
        ...
        MountItem item = mountContent(index, component, content, host, layoutOutput);
    ...
    public static ComponentClickListener getComponentClickListener(View v) {
    ...
    public static void setComponentClickListener(View v, ComponentClickListener listener) {
        if (v instanceof ComponentHost) {
    ...
    public static ComponentLongClickListener getComponentLongClickListener(View v) {
        if (v instanceof ComponentHost) {
    ...
    public static void setComponentLongClickListener(View v, ComponentLongClickListener listener) {
        if (v instanceof ComponentHost) {
    ...
    public static ComponentFocusChangeListener getComponentFocusChangeListener(View v) {
        if (v instanceof ComponentHost) {
    ...
    public static void setComponentFocusChangeListener(View v, ComponentFocusChangeListener listener) {
        if (v instanceof ComponentHost) {
    ...
    public static ComponentTouchListener getComponentTouchListener(View v) {
        if (v instanceof ComponentHost) {
    ...
    public static void setComponentTouchListener(View v, ComponentTouchListener listener) {
        if (v instanceof ComponentHost) {
    ...
    private void unmountDisappearingItemChild(ComponentContext context, MountItem item) {
        ...
        if (item.getComponent().hasChildLithoViews()) {
            int index = mCanMountIncrementallyMountItems.indexOfValue(item);
            if (index > 0) {
    ...
    private void assertNoDanglingMountContent(MountItem item) {
        int index = mIndexToItemMap.indexOfValue(item);
        if (index > -1) {
            ComponentsReporter.emitMessage(
                ComponentsReporter.LogLevel.ERROR,
    ...
    private void unmountItem(int index, LongSparseArray<ComponentHost> hostsByMarker) {
        ...
        if ((content instanceof ComponentHost) && !(content instanceof LithoView)) {
            ...
            for (int i = host.getMountItemCount() - 1; i >= 0; i--) {
                MountItem mountItem      = host.getMountItemAt(i);
                long      layoutOutputId = mIndexToItemMap.keyAt(mIndexToItemMap.indexOfValue(mountItem));
        ...
        if (content instanceof HasLithoViewChildren) {
            ArrayList<LithoView> lithoViews = new ArrayList<>();
        ...
        long layoutOutputId = mLayoutOutputsIds[index];
    ...
    private void endUnmountDisappearingItem(OutputUnitsAffinityGroup<MountItem> group) {
        ...
        for (int i = 0, size = group.size(); i < size; i++) {
            ...
            if (group.typeAt(i) == OutputUnitType.HOST) {
                ComponentHost content = (ComponentHost) item.getContent();
    ...
    private void updateDisappearingMountItems(LayoutState newLayoutState) {
        Map<TransitionId, ?> nextMountedTransitionIds = newLayoutState.getTransitionIdMapping();
        for (TransitionId transitionId : nextMountedTransitionIds.keySet()) {
            OutputUnitsAffinityGroup<MountItem> disappearingItem =
    ...
    private void createNewTransitions(LayoutState newLayoutState, Transition rootTransition) {
        ...
        Map<TransitionId, ?> nextTransitionIds = newLayoutState.getTransitionIdMapping();
    ...
    private void regenerateAnimationLockedIndices(LayoutState newLayoutState) {
        Map<TransitionId, OutputUnitsAffinityGroup<LayoutOutput>> transitionMapping =
        ...
        if (transitionMapping != null) {
            for (Map.Entry<TransitionId, OutputUnitsAffinityGroup<LayoutOutput>> transition :
                ...
                OutputUnitsAffinityGroup<LayoutOutput> group = transition.getValue();
    ...
    private void updateAnimationLockCount(LayoutState layoutState, int index, boolean increment) {
        // Update children
        int lastDescendantIndex = findLastDescendantIndex(layoutState, index);
    ...
    private boolean performIncrementalMount(
        ...
        if (localVisibleRect.top > 0 || mPreviousLocalVisibleRect.top > 0) {
            ...
            while (mPreviousBottomsIndex < count
                ...
                long id                = layoutOutputBottoms.get(mPreviousBottomsIndex).getId();
                int  layoutOutputIndex = layoutState.getLayoutOutputPositionForId(id);
            ...
            while (mPreviousBottomsIndex > 0
                ...
                mPreviousBottomsIndex--;
                LayoutOutput layoutOutput = layoutOutputBottoms.get(mPreviousBottomsIndex);
        ...
        int height = mLithoView.getHeight();
        if (localVisibleRect.bottom < height || mPreviousLocalVisibleRect.bottom < height) {
            ...
            while (mPreviousTopsIndex < count
                ...
                LayoutOutput layoutOutput = layoutOutputTops.get(mPreviousTopsIndex);
                int layoutOutputIndex =
                    layoutState.getLayoutOutputPositionForId(layoutOutput.getId());
                if (getItemAt(layoutOutputIndex) == null) {
                    mountLayoutOutput(
            ...
            while (mPreviousTopsIndex > 0
                ...
                mPreviousTopsIndex--;
                long id                = layoutOutputTops.get(mPreviousTopsIndex).getId();
                int  layoutOutputIndex = layoutState.getLayoutOutputPositionForId(id);
                if (!isAnimationLocked(layoutOutputIndex)) {
                    unmountItem(layoutOutputIndex, mHostsByMarker);
                }
        ...
        for (int i = 0, size = mCanMountIncrementallyMountItems.size(); i < size; i++) {
            MountItem mountItem      = mCanMountIncrementallyMountItems.valueAt(i);
            long      layoutOutputId = mCanMountIncrementallyMountItems.keyAt(i);
            if (!mComponentIdsMountedInThisFrame.contains(layoutOutputId)) {
                int layoutOutputPosition = layoutState.getLayoutOutputPositionForId(layoutOutputId);
    ...
    private static @Nullable void collectMountTimeTransitions(
        ...
        List<Component> componentsNeedingPreviousRenderData =
    ...
    public Deque<TestItem> findTestItems(String testKey) {
        if (mTestItemMap == null) {
        ...
        Deque<TestItem> items = mTestItemMap.get(testKey);
