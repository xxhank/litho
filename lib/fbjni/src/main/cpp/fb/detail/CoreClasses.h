...
/// Wrapper to provide functionality to jclass references
        ...
        class JClass : public JavaClass<JClass, JObject, jclass> {
            ...
            template<typename T>
            JField<enable_if_t<IsJniScalar<T>(), T>>
            getField(const char *name, const char *descriptor) const;
            ...
            template<typename F>
            JNonvirtualMethod<F>
            getNonvirtualMethod(const char *name, const char *descriptor) const;
...
// should use JArrayClass<jobject>.
        ...
        template<typename Target>
        local_ref<typename Target::javaentry>
        adopt_local(detail::ElementProxy<Target> elementProxy) {
...
/// the Java garbage collector.
        ...
        struct JStackTraceElement : JavaClass<JStackTraceElement> {
            ...
            static local_ref<javaobject>
            create(const std::string &declaringClass, const std::string &methodName, const std::string &file, int line);
